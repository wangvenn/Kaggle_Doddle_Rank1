...
for filename in files:
    ...
    df_s_time = df_s[['key_id']].copy()
    f = partial(_add_time, df_r, df_s)
    pool = multiprocessing.Pool(multiprocessing.cpu_count())
    df_s_time['time'] = pool.map(f, range(df_s_time.shape[0]))
